...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/5e/c5el5wf27ffc7hdns27uu3m4ybwrdedwutaftgnb6tnodo7chnjl.py
...
#   %add : Tensor "bf16[4096, 8, 1024][8192, 1024, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_1, %primals_2), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            buf0 = empty_strided_cuda((4096, 8, 1024), (8192, 1024, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [out_1], Original ATen: [aten.add]
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_0.run(primals_1, primals_2, buf0, 33554432, stream=stream2)
            del primals_1
            del primals_2
        return (buf0, )
