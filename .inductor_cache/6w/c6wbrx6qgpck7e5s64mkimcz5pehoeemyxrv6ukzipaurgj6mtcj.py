...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/ew/cewon5hqep2h5b6wxy7bhkidiatacob3p7xh72752rlv3i4evahw.py
...
#   y => add
# Graph fragment:
#   %primals_1 : Tensor "bf16[4096, 4, 4352][17408, 4352, 1]cuda:3" = PlaceHolder[target=primals_1]
#   %primals_2 : Tensor "bf16[4352][1]cuda:3" = PlaceHolder[target=primals_2]
#   %add : Tensor "bf16[4096, 4, 4352][17408, 4352, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_1, %primals_2), kwargs = {})
#   %split : [num_users=2] = call_function[target=torch.ops.aten.split.Tensor](args = (%add, 2176, -1), kwargs = {})
#   %convert_element_type : Tensor "f32[4096, 4, 2176][8704, 2176, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%getitem, torch.float32), kwargs = {})
#   %sigmoid : Tensor "f32[4096, 4, 2176][8704, 2176, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sigmoid.default](args = (%convert_element_type,), kwargs = {})
#   %mul : Tensor "f32[4096, 4, 2176][8704, 2176, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type, %sigmoid), kwargs = {})
#   %convert_element_type_1 : Tensor "bf16[4096, 4, 2176][8704, 2176, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul, torch.bfloat16), kwargs = {})
#   %mul_1 : Tensor "bf16[4096, 4, 2176][8704, 2176, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_1, %getitem_1), kwargs = {})
#   return %mul_1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            triton_poi_fused_add_mul_silu_split_0.run(primals_1, primals_2, buf0, 35651584, stream=stream3)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_1 = rand_strided((4096, 4, 4352), (17408, 4352, 1), device='cuda:3', dtype=torch.bfloat16)
    primals_2 = rand_strided((4352, ), (1, ), device='cuda:3', dtype=torch.bfloat16)
