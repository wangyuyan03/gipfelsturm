...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/mt/cmtxvqrir7jeu4m4vghwgs5i4uuhzgtai2rgef6y3qaonkzwr3c5.py
...
# Graph fragment:
#   %arg2_1 : Tensor "i64[4096, 4][4, 1]cuda:2" = PlaceHolder[target=arg2_1]
#   %bitwise_or : Tensor "b8[4096, 4][4, 1]cuda:2" = PlaceHolder[target=bitwise_or]
#   %index_put : Tensor "i64[4096, 4][4, 1]cuda:2" = PlaceHolder[target=index_put]
#   %copy_ : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2" = PlaceHolder[target=copy_]
#   %arg1_1 : Tensor "f32[4096, 4][4, 1]cuda:2" = PlaceHolder[target=arg1_1]
#   %unsqueeze : Tensor "f32[4096, 4, 1][4, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg1_1, -1), kwargs = {})
#   %sub : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.sub.Tensor](args = (%arg0_1, %unsqueeze), kwargs = {})
#   %view_2 : Tensor "f32[16384, 50304][50304, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%sub, [-1, 50304]), kwargs = {})
#   %iota : Tensor "i64[16384][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (16384,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:2, requires_grad: False})
#   %sub_1 : Tensor "i64[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%arg2_1, 0), kwargs = {})
#   %lt : Tensor "b8[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.lt.Scalar](args = (%arg2_1, 0), kwargs = {})
#   %ge : Tensor "b8[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.ge.Scalar](args = (%arg2_1, 50304), kwargs = {})
#   %bitwise_or : Tensor "b8[4096, 4][4, 1]cuda:2"[num_users=3] = call_function[target=torch.ops.aten.bitwise_or.Tensor](args = (%lt, %ge), kwargs = {})
...
#   %index_put : Tensor "i64[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%sub_1, [%bitwise_or], %full_default), kwargs = {})
#   %view_3 : Tensor "i64[16384][1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%index_put, [-1]), kwargs = {})
#   %index : Tensor "f32[16384][1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index.Tensor](args = (%view_2, [%iota, %view_3]), kwargs = {})
#   %view_4 : Tensor "f32[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%index, [4096, 4]), kwargs = {})
...
#   %index_put_1 : Tensor "f32[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%view_4, [%bitwise_or], %full_default_1), kwargs = {})
...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/hi/chi6ffntsakbvapfak52e2dydzcb3dquqpe4nc34zfu666asdsph.py
...
#   vocab_parallel_logits => sub
# Graph fragment:
#   %buf3 : Tensor "f32[4096, 4][4, 1]cuda:2" = PlaceHolder[target=buf3]
#   %index : Tensor "f32[16384][1]cuda:2" = PlaceHolder[target=index]
#   %buf4 : Tensor "f32[4096, 4][1]cuda:2" = PlaceHolder[target=buf4]
#   %unsqueeze : Tensor "f32[4096, 4, 1][4, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg1_1, -1), kwargs = {})
#   %sub : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.sub.Tensor](args = (%arg0_1, %unsqueeze), kwargs = {})
#   %view_2 : Tensor "f32[16384, 50304][50304, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%sub, [-1, 50304]), kwargs = {})
#   %iota : Tensor "i64[16384][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (16384,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:2, requires_grad: False})
...
#   %full_default_1 : Tensor "f32[][]cpu"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cpu, pin_memory: False})
#   %index_put_1 : Tensor "f32[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%view_4, [%bitwise_or], %full_default_1), kwargs = {})
#   %cat : Tensor "f32[8192, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.cat.default](args = ([%index_put_1, %sum_1],), kwargs = {})
#   return %buf4,%buf6
...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/qs/cqsg3h55q57cveylnmim7fmru7udjoi6th2mzkkyudmkebyocqcq.py
...
#   %copy_ : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2" = PlaceHolder[target=copy_]
#   %arg1_1 : Tensor "f32[4096, 4][4, 1]cuda:2" = PlaceHolder[target=arg1_1]
#   %exp : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2" = PlaceHolder[target=exp]
#   %buf4 : Tensor "f32[4096, 4][1]cuda:2" = PlaceHolder[target=buf4]
#   %buf3 : Tensor "f32[4096, 4][4, 1]cuda:2" = PlaceHolder[target=buf3]
#   %sum_1 : Tensor "f32[4096, 4][4, 1]cuda:2" = PlaceHolder[target=sum_1]
#   %unsqueeze : Tensor "f32[4096, 4, 1][4, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%arg1_1, -1), kwargs = {})
#   %sub : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.sub.Tensor](args = (%arg0_1, %unsqueeze), kwargs = {})
#   %exp : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.exp.default](args = (%sub,), kwargs = {})
#   %sum_1 : Tensor "f32[4096, 4][4, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%exp, [-1]), kwargs = {})
#   %copy_ : Tensor "f32[4096, 4, 50304][201216, 50304, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.copy_.default](args = (%arg0_1, %exp), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_arange_bitwise_or_ge_index_index_put_lift_fresh_lt_sub_unsqueeze_view_0.run(arg2_1, arg0_1, arg1_1, buf0, buf1, buf2, buf3, 16384, stream=stream2)
            del arg2_1
            buf7 = empty_strided_cuda((8192, 4), (4, 1), torch.float32)
            buf6 = reinterpret_tensor(buf7, (4096, 4), (4, 1), 0)  # alias
            # Topologically Sorted Source Nodes: [unsqueeze, vocab_parallel_logits, predicted_logits_1d, arange_1d, predicted_logits, setitem_1, predicted_logits_sum_exp_logits], Original ATen: [aten.unsqueeze, aten.sub, aten.view, aten.arange, aten.index, aten.lift_fresh, aten.index_put, aten.cat]
            stream2 = get_raw_stream(2)
            triton_poi_fused_arange_cat_index_index_put_lift_fresh_sub_unsqueeze_view_1.run(buf3, buf2, buf6, 16384, stream=stream2)
            buf5 = reinterpret_tensor(buf7, (4096, 4), (4, 1), 16384)  # alias
            buf9 = empty_strided_cuda((4096, 4, 50304), (201216, 50304, 1), torch.float32)
            # Topologically Sorted Source Nodes: [unsqueeze, vocab_parallel_logits, exp, sum_exp_logits], Original ATen: [aten.unsqueeze, aten.sub, aten.exp, aten.sum, aten.copy_]
            stream2 = get_raw_stream(2)
            triton_red_fused_copy__exp_sub_sum_unsqueeze_2.run(arg0_1, arg1_1, buf5, buf9, arg0_1, 16384, 50304, stream=stream2)
            del arg1_1
            del buf2
            del buf3
            del buf9
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = rand_strided((4096, 4, 50304), (201216, 50304, 1), device='cuda:2', dtype=torch.float32)
    arg1_1 = rand_strided((4096, 4), (4, 1), device='cuda:2', dtype=torch.float32)
    arg2_1 = rand_strided((4096, 4), (4, 1), device='cuda:2', dtype=torch.int64)
