...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/yv/cyvtvqszb73h4qvpfynn74nl4au2f6e5ieenslqo3g7jwbpsa6tf.py
...
#   split => split
# Graph fragment:
#   %arg0_1 : Tensor "f32[8192, 8][8, 1]cuda:2" = PlaceHolder[target=arg0_1]
#   %split : [num_users=2] = call_function[target=torch.ops.aten.split.Tensor](args = (%arg0_1, 4096), kwargs = {})
#   %log : Tensor "f32[4096, 8][8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.log.default](args = (%getitem_1,), kwargs = {})
#   %sub : Tensor "f32[4096, 8][8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%log, %getitem), kwargs = {})
#   return %sub
...
# kernel path: /iopsstor/scratch/cscs/course_00313/gipfelsturm/.inductor_cache/n3/cn353fachtk5imd4uwjuvfqwoq73mkapkzkn6os7bimzd7dezxo6.py
...
#   %arg0_1 : Tensor "f32[8192, 8][8, 1]cuda:2" = PlaceHolder[target=arg0_1]
#   %div : Tensor "f32[4096, 8, 50304][402432, 50304, 1]cuda:2" = PlaceHolder[target=div]
#   %split : [num_users=2] = call_function[target=torch.ops.aten.split.Tensor](args = (%arg0_1, 4096), kwargs = {})
#   %unsqueeze : Tensor "f32[4096, 8, 1][8, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%getitem_1, -1), kwargs = {})
#   %div : Tensor "f32[4096, 8, 50304][402432, 50304, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.div.Tensor](args = (%arg1_1, %unsqueeze), kwargs = {})
#   %copy_ : Tensor "f32[4096, 8, 50304][402432, 50304, 1]cuda:2"[num_users=0] = call_function[target=torch.ops.aten.copy_.default](args = (%arg1_1, %div), kwargs = {})
#   return %div,%buf2
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_log_split_sub_0.run(arg0_1, buf0, 32768, stream=stream2)
            # Topologically Sorted Source Nodes: [split, unsqueeze, div_], Original ATen: [aten.split, aten.unsqueeze, aten.div, aten.copy_]
            stream2 = get_raw_stream(2)
            triton_poi_fused_copy__div_split_unsqueeze_1.run(arg1_1, arg0_1, arg1_1, 1648361472, stream=stream2)
            del arg0_1
            del arg1_1
        return (buf0, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = rand_strided((8192, 8), (8, 1), device='cuda:2', dtype=torch.float32)
    arg1_1 = rand_strided((4096, 8, 50304), (402432, 50304, 1), device='cuda:2', dtype=torch.float32)
