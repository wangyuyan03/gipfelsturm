...
def plot_throughput(runs, out, plt):
    ...
    # Scaling efficiency (GBS=256, varying nodes) — plot all models that have
    # multi-node data so the chart shows why 760M was chosen over 350M.
    model_node_data = {}
    for r in thr:
        if "gbs256" not in r.job_name:
            continue
        for model in ("125m", "350m", "760m", "1.5b", "3b"):
            if model not in r.job_name:
                continue
            m = re.search(r"(\d+)n", r.job_name)
            if m:
                model_node_data.setdefault(model, {})[int(m.group(1))] = r.avg_tokens_per_sec_gpu

    # Only plot models that have data on at least 2 different node counts
    multi_node_models = {k: v for k, v in model_node_data.items() if len(v) >= 2}

    if multi_node_models:
        colors = {"350m": "#FF9800", "760m": "#2196F3",
                  "125m": "#9C27B0", "1.5b": "#4CAF50", "3b": "#F44336"}
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 4))

        all_nodes = sorted(set(n for nd in multi_node_models.values() for n in nd))

        ax2.plot(all_nodes, [100.0] * len(all_nodes), "--",
                 color="#9E9E9E", linewidth=1.5, label="Ideal")

        for model, nd in sorted(multi_node_models.items()):
            nodes    = sorted(nd)
            baseline = nd[nodes[0]]
            raw = [nd[n] / 1000 for n in nodes]
            eff = [nd[n] / baseline * 100 for n in nodes]
            c   = colors.get(model, "#607D8B")

            ax1.plot(nodes, raw, "o-", color=c, linewidth=2,
                     markersize=7, label=model.upper())
            for n, v in zip(nodes, raw):
                ax1.annotate("{:.1f}k".format(v), (n, v),
                             textcoords="offset points", xytext=(0, 8),
                             ha="center", fontsize=8)

            ax2.plot(nodes, eff, "o-", color=c, linewidth=2,
                     markersize=7, label=model.upper())
        ...
        fig.suptitle("Multi-Node Scaling Efficiency  (GBS=256, seq=4096)", y=1.02)
        fig.tight_layout()
